...
void (*morph_dma_force_trigger_function)(uint8_t unit, uint8_t channel) = &MorphUtils_empty_callback;
...
void MorphDMA_register_enable_force_trigger_function(void (*function)(uint8_t unit, uint8_t channel))
{
	morph_dma_force_trigger_function = function;
}
...
void MorphDMA_force_trigger(uint8_t unit, uint8_t channel)
{
	#ifdef MORPH_IMPLICIT
	morph_config_dma_force_trigger(unit, channel);
	#else
	morph_dma_force_trigger_function(unit, channel);
	#endif
}
...
{
#ifdef MORPH_IMPLICIT
	morph_config_dma_enable_transfer_error_callback(unit, channel, enabled);
#else
...
{
#ifdef MORPH_IMPLICIT
	morph_config_dma_set_transfer_error_callback(unit, channel, callback);
#else
