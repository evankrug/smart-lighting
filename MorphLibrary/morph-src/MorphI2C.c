...
void MorphI2C_write(MorphI2CChannel c, MorphI2CByte b)
{
	...
	#ifdef MORPH_IMPLICIT
		morph_config_i2c_write(c, b);
	#else
		morph_write_function(c, b);
	#endif	
...
{
	#ifdef MORPH_IMPLICIT
	morph_config_i2c_set_scl_pin(c, pn);
	#else
	morph_set_scl_pin_function(c, pn);
	#endif	
}
...
{
	#ifdef MORPH_IMPLICIT
	morph_config_i2c_set_sda_pin(c, pn);
	#else
	morph_set_sda_pin_function(c, pn);
	#endif	
}
