...
void MorphTime_init_rtc(void);

MorphTimeValue MorphTime_millis(void);

// This is to set a master "guide clock" that the rtc/timer clock will try to maintain itself to. 
void MorphTime_set_master_interrupt(MorphPinNumber pn,  enum MorphExternalInterruptType it, uint16_t frequency);
