...
void morph_config_gpio_set_pin_mode(MorphPinNumber pn, enum MorphPinMode pm);
...
volatile void morph_config_uart_send_byte(MorphUARTChannel c, MorphUARTByte byte);
...
void morph_config_time_init_rtc(void);

MorphTimeValue morph_config_time_millis(void);
...
void morph_config_pwm_set_pwm_duty_float(MorphPinNumber pn, double duty);
...
void morph_config_dma_force_trigger(uint8_t unit, uint8_t channel);

// ---- DMA END ----


void morph_config_init(void);
